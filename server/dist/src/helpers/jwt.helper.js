...
let JwtHelper = class JwtHelper {
    ...
    verifyTokens(payload) {
        try {
            ...
            const refreshToken = this.jwt.verify(payload.refreshToken, {
                secret: process.env.REFRESH_TOKEN_SECRET,
            });
            return {
                id: accessToken.id,
            ...
            if (error instanceof jwt_1.TokenExpiredError) {
                try {
                    const refreshPayload = this.jwt.verify(payload.refreshToken, {
                        secret: process.env.REFRESH_TOKEN_SECRET,
                    });
                    const newAccessToken = this.jwt.sign({ id: refreshPayload.id, role: refreshPayload.role }, {
                        secret: process.env.ACCESS_TOKEN_SECRET,
                        expiresIn: process.env.ACCESS_TOKEN_TIME,
                    });
                    const newRefreshToken = this.jwt.sign({ id: refreshPayload.id, role: refreshPayload.role }, {
                        secret: process.env.REFRESH_TOKEN_SECRET,
                        expiresIn: process.env.REFRESH_TOKEN_TIME,
                    });
                    const decodedAccess = this.jwt.verify(newAccessToken, {
                        secret: process.env.ACCESS_TOKEN_SECRET,
                    });
                    return {
